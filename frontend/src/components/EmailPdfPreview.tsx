...
import { fetchEmailAttachmentBlob, getEmailAttachmentUrl } from '@/lib/api';

export interface EmailPdfPreviewProps {
  open: boolean;
...
export function EmailPdfPreview({
  ...
  useEffect(() => {
    ...
    fetchEmailAttachmentBlob(emailId, attachmentId)
      .then((blob) => {
        if (cancelled) return;
  ...
  if (!open || !emailId || !attachmentId) return null;

  const openInTabUrl = getEmailAttachmentUrl(emailId, attachmentId);
